...
| **07** | `variant/07-logo-resonance-distributed` | **Basis V02 (Cinematic Hero) + Logo-Resonanz als verteiltes Markenmotiv** mit Taste-Skill-Crescendo. Die Animation erscheint 5× über die Seite, gestaffelt nach Größe/Deckkraft/Anschnitt/Position: Hero top-right (620 px, 3,5 %, kaum sichtbar im dunklen Cinema) → KI-Werkzeug links Mitte (300 px, 7 %) → Für-wen top-left (360 px, 5 %) → Grundwerte top-right (420 px, 8 %) → Final-CTA bottom-right (540 px, 12 %, Climax). LogoResonanz-Komponente erweitert (`vertical: middle`, separate `insetX/insetY`), jede Instanz mit eigener `instanceId`. | Wie V02 | Wie V02 | Live | [Desktop](variants/screenshots/07-desktop.png) · [Mobile](variants/screenshots/07-mobile.png) |
...
## Freie Neuentwürfe — Variante 14–17
...
14–16 sind die lauten, mutigen Entwürfe; **17** ist bewusst die *leise*, dezente
Antwort mit einer einzigen besonderen Interaktion (der scroll-gezeichnete „Faden").
...
| **17** | `17-faden` | **Der Faden — Serpentinen-Scrollpfad** — die *leise*, edle Variante (kein Three.js, nur GSAP + SVG). Ein einziger, sich sanft schlängelnder Faden (zarter Kupfer→Quellwasser-Verlauf) zeichnet sich beim Scrollen vertikal durch die helle Pergament-Seite; sein leuchtender Kopf-Knoten hängt auf ~62 % Viewport-Höhe und **enthüllt jeden Abschnitt erst, wenn er ihn erreicht** (Reveal synchron zum Faden-Kopf über eine vorberechnete y→Länge-Lookup-Tabelle). Die Wellenlinien-Idee aus V15, hier zum tragenden „roten Faden" gemacht. Fraunces + Inter. |
